from typing import List, Dict
from langchain_core.prompts import PromptTemplate
from langchain.messages import HumanMessage, SystemMessage, AIMessage

class PromptGenerator:

    @staticmethod
    def output_templete():
        # Fixed string formatting and removed unnecessary concatenation
        return (
            "text will be short do not describe so long."
            "response use proper html tag like <p>, <tb>, <tr>, <ul>, <li>,<a>, <img>, bold , italic "
            "do not use h1 or h2 like big tag"
        )

    @staticmethod   
    def GeneralPrompt(user_query, relevent_info, previous_chat: List[Dict]):
        sys_message = SystemMessage(
            content=(
                "You are an assistant for a boiler company that sells boilers, boiler controllers, and related products. "
                "Your responsibility is to answer customer queries about these products and use available tools when necessary. "
                "Provide accurate, concise, and relevant answers. "
                # "text will be short do not describe so long."
                "Do not hallucinate or make up information. "
                "If the question is outside the scope of boilers, boiler controllers, or related products, politely respond with a brief apology and state that you can only assist with company-related products. "
                f"follow this template {PromptGenerator.output_templete()}"
                "if user want quote then they first need to must attent the quiz. "
                "one start a quiz then they must need to complete it utill they so stop the quiz."
                "After compete the quiz ask them about there personal information"
                "During or untill completed the quiz use QuizTool tool"
                "If the user says Start Quiz or clearly confirms they want to begin the quiz, call QuizTool before answering. "
                "Do not invent quiz questions; the quiz must use the QuizTool questions_and_options order. "
                "Never answer a quiz start or quiz step with placeholder text like Loading your first question. "
                "if user completed the quote quiz and then create quote other ways not"
                "and if quiz and quote bothe are completed give your code like this "

                "<a href= {https://arronwh-website.vercel.app/boilers/system-selection?quoteId=_id} View your code>" 
                "Data will collect on this following order and one question at a time them finally fill the postcode_form"
        
                "Data will collect on this following order and one question at a time them finally fill the postcode_form"
            )
        )

        hum_message = HumanMessage(
            content=f'Current user Query: {user_query} \n\nRelevant information from RAG: {relevent_info} Previous Chat: {previous_chat}'
        )
        
        temp = PromptTemplate(template="System instruction: {sys_message}\n{hum_message}",
                             input_variables=['sys_message', 'hum_message'])
        
        prompt = temp.invoke(
            {
                'sys_message': sys_message.content,
                'hum_message': hum_message.content
            }
        )
        return str(prompt)

    @staticmethod   
    def ToolsPrompt(user_query, tools_data: List[Dict]):
        sys_message = SystemMessage(
            content=(
                "You are an assistant of the boiler company. "
                "Your task is to analyze tools data based on user query."
                "Data will collect on this following order and one question at a time them finally fill the postcode_form"
                # "text will be short do not describe so long."
            f"follow this template {PromptGenerator.output_templete()}"
            )
        )

        hum_message = HumanMessage(
            content=f'Current user Query: {user_query} \n\nTools data: {tools_data}'
        )
        
        temp = PromptTemplate(template="System instruction: {sys_message}\n{hum_message}",
                             input_variables=['sys_message', 'hum_message'])
        
        prompt = temp.invoke(
            {
                'sys_message': sys_message.content,
                'hum_message': hum_message.content
            }
        )
        return str(prompt)
    
    
    @staticmethod   
    def QuizToolsPrompt(user_query, tools_data: str, previous_chat: List[Dict]):
        sys_message = SystemMessage(
            content=(
                "You are an assistant of the boiler company. "
                "Your task is to analyze tools data based on user query."
                # "text will be short do not describe so long."
                "if user want quote then they must attent the quiz. "
                "Before start quiz first ask to confirm quiz. "
                "Quiz questions must come sequentially using only the questions_and_options from Tools data. "
                "Never create, rename, skip, or replace quiz questions or options. "
                "Never answer with placeholder text like Loading your first question. "
                "If the user confirms with Start Quiz, start at serial_no 1 exactly: Are you a homeowner or landlord? Options: Homeowner, Landlord. "
                "After each answer, use the next rule from Tools data to choose the next question. "
                "one start a quiz then they must need to complete it utill they so stop the quiz."
                "After compete the quiz ask them aboout there personal information"
                "Data will collect on this following order and one question at a time them finally fill the postcode_form"
                f"follow this template {PromptGenerator.output_templete()}"
            )
        )

        hum_message = HumanMessage(
            content=f'Current user Query: {user_query} \n Previous chat: {previous_chat} \n\nTools data: {tools_data}'
        )
        
        temp = PromptTemplate(template="System instruction: {sys_message}\n{hum_message}",
                             input_variables=['sys_message', 'hum_message'])
        
        prompt = temp.invoke(
            {
                'sys_message': sys_message.content,
                'hum_message': hum_message.content
            }
        )
        return str(prompt)
    
    @staticmethod   
    def CreateQuotePrompt(user_query, tools_data: str, previous_chat: List[Dict]):
        sys_message = SystemMessage(
            content=(
                "You are an assistant of the boiler company. "
                "Your task is to analyze tools data based on user query."
                # "text will be short do not describe so long."
                "if user completed the quote and user postcode_data then create quote"
                "and give your code like this "
                "<a href= {https://arronwh-website.vercel.app/boilers/system-selection?quoteId=_id} View your code>" 
                "Data will collect on this following order and one question at a time them finally fill the postcode_form"
                f"follow this template {PromptGenerator.output_templete()}"
            )
        )

        hum_message = HumanMessage(
            content=f'Current user Query: {user_query} \n Previous chat: {previous_chat} \n\nTools data: {tools_data}'
        )
        
        temp = PromptTemplate(template="System instruction: {sys_message}\n{hum_message}",
                             input_variables=['sys_message', 'hum_message'])
        
        prompt = temp.invoke(
            {
                'sys_message': sys_message.content,
                'hum_message': hum_message.content
            }
        )
        return str(prompt)
